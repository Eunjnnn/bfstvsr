...
from models.modules.STFS import STFSFourierMapper
from models.modules.STBS_inference import STBSplineMapper
...
class LunaTokis(nn.Module):
    ...
    def forward(self, x, input_target_frames, target_t, scale=None, rank=0, train_idx=0, use_GT=True, iter=12, flows=None):
        ...
        vx, vy, eps_shift = 0, 0, 0

        hr_coord = make_coord((HH, WW)).unsqueeze(0)
        hr_coord = hr_coord.to(feat.device)
        eps_shift = 1e-6
        ...
        c1, c3, c4, c5 = 2 * B * warpped_feat.shape[1], 0, feat_coord.shape[1], residual.shape[1] * B
        '''
        to_be_warp (LR)
        ...
        to_be_warp = torch.cat(
            (warpped_feat.reshape(1, c1, H, W), feat_coord.reshape(1, c4, H, W), residual.reshape(
        ...
        q_flow_feat =  torch.cat([q_feat.reshape(2*B,qs,-1),
                                    rel_coord.repeat(2*B,1,1),
                                    ], dim=-1)
                          
        q_feat = self.imnet(q_feat, rel_coord.repeat(2*B,1,1).reshape(2*B, HH, WW, -1))

        q_feat = q_feat.reshape(2 * B, HH, WW, -1).permute(0, 3, 1, 2)
        q_feat_low = q_feat_low.reshape(2 * B, HH, WW, -1).permute(0, 3, 1, 2)
        ...
        # start = time.time()
        flow_feat = self.flow_imnet(q_flow_feat, ts, init=True)
        
        # print(time.time()-start)     
        flow = flow_feat.reshape(2 * B, HH, WW, -1).permute(0, 3, 1, 2)
        
        if self.training:
            flow_GT = flow_GT.reshape(2 * B, -1, HH, WW)
            
        feat = torch.cat([
            q_feat.reshape(2 * B, -1, HH, WW), 
            flow[:, :-1].detach(),
            q_feat_low.reshape(2 * B, -1, HH, WW)
        ], 1)
        ...
        if use_GT:
            ...
            # torch.Size([2, 130, 720, 1280]) torch.Size([2, 2, 720, 1280]) torch.Size([2, 1, 720, 1280])
            output, warped_z = self.fwarp(feat, flow, z)
            z_max = self.fwarp_max(z.exp(), flow).detach()
            count = self.fwarp_count(z, flow).detach()
            
        output = output.reshape(2, B, -1, HH, WW).sum(0)
        warped_z = warped_z.reshape(2, B, -1, HH, WW).sum(0)
        warped_z[warped_z == 0] = 1.0
        output /= warped_z
        z_max = z_max.reshape(2, B, -1, HH, WW).max(0)[0]
        count = count.reshape(2, B, -1, HH, WW).sum(0)
        ...
        output_all = torch.cat((
            output.reshape(B, -1, HH, WW),
            extra.reshape(B, -1, HH, WW),
            q_residual.reshape(B, -1, HH, WW),
            ts.reshape(B, 1, 1, 1).repeat(1, 1, HH, WW),
        ), 1).reshape(B, -1, HH, WW)
        
        output_all = self.synth_net(output_all.reshape(B, -1, HH * WW).permute(0, 2, 1)).permute(0, 2, 1).reshape(B, -1, HH, WW)

        preds.append(output_all)
        ...
        for c in range(N)[1:]:
            ...
            # predict with imnet and flow_imnet
            # start = time.time()
            flow_feat = self.flow_imnet(q_flow_feat, ts)
        ...
        output_all = torch.stack(preds, dim=0)
        torch.cuda.empty_cache()

        return torch.clamp((output_all), 0, 1)
