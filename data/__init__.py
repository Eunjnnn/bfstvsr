...
def create_dataset(dataset_opt):
    ...
    if mode == 'Adobe_a':
        ...
        from data.Vimeo7_dataset_test import Vimeo7Dataset as D
    else:
...
def collate_function(data):
    ...
    # print("[[img_GTs.shape[-2]], [img_GTs.shape[-1]]] : ", [[img_GTs.shape[-2]], [img_GTs.shape[-1]]])
...
def collate_function_test(data):
    ...
    time_t = [torch.cat([time_[2][i][None] for time_ in data], dim=0) for i in range(len(data[0][2]))]
    folder_name = os.path.basename(os.path.dirname(data[0][3][0]))
    return {'LQs': img_LQs, 'GT': img_GTs, 'scale': [[img_GTs.shape[-2]], [img_GTs.shape[-1]]], 'time': time_t, 'folder': folder_name} 